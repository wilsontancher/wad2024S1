...
router.post('/api/events', function (req, res) {
    let data = req.body;
    let organizerId = res.locals.organizerId;
    db.addEvent(data.name,  data.description,  data.startDate,  data.startTime,  data.endDate,  data.endTime, organizerId)
    .then(function(response){
...
router.put('/api/events/:id',function(req,res) {
    ...
    .then(function(response){
        if(!response) {
            ...
            //need to use the equals function for comparison because the organizer IDs are two different objects
            if(!response.organizer._id.equals(organizerId)) {
                res.status(403).json({"message":"You are not authorized to perform this action."});
            } else {
                db.updateEventById(id,data.name,  data.description,  data.startDate,  data.startTime,  data.endDate,  data.endTime)
                .then(function(response){
                    res.status(200).json({"message":response});
                })
                .catch(function(error){
...
router.delete('/api/events/:id',function(req,res){
    ...
    .then(function(response){
        if(!response) {
            res.status(200).json({"message":"Unable to find record to delete"});
        } else {
            //need to use the equals function for comparison because the organizer IDs are two different objects
            if(!response.organizer._id.equals(organizerId)) {
                res.status(403).json({"message":"You are not authorized to perform this action."});
...
router.post('/api/events/search',function(req,res){
    let data = req.body;
    db.searchEvents(data.name)
    .then(function(response){
        res.status(200).json(response);
    })
    .catch(function(error){
